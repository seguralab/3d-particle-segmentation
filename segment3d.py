...
if __name__ == '__main__':
    ...
    list_labels = watershed_again(labels, local_maxi, d_peak, peak_prom, empty_ext, s2v_max, \
    footprint_direct, footprint_indirect)
    # list of number of voxels of each bead
    list_voxel_count = np.array([label.sum() for label in list_labels])
    ...
    labels = np.zeros(labels.shape, dtype = 'uint16')
    for (ii, ind) in enumerate(order):
        labels += (ind*list_labels[ii]).astype(labels.dtype)
    ...
    if further_smooth:
        for (ii, label) in enumerate(list_labels):
            list_labels[ii] = smooth_region(label, footprint_neighbor)
        ...
        labels = np.zeros(labels.shape,dtype = 'uint16')
        for (ii, label) in enumerate(list_labels):
            labels += (label*(order[ii])).astype(labels.dtype)
        if does_plot:
            img = labels[:,:,example_frame].astype('int16')
            num_beads = local_maxi.max()
            img[img<=0] = 0-num_beads//5
    ...
    (list_bead_voxel_count, list_bead_data) = list_sparse(list_labels)
