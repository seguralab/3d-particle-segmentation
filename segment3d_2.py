...
# from resize_leica import convert_lif
from resize_tif import convert_tif, get_pixel_and_z_dimensions
...
if __name__ == '__main__':
    ...
    # FOR LEICA FILES - uncomment below and comment out Lines 130

    # filename = '.\\lif_files\\DeLaporte\\L2_scan_water.lif'
    # fluorescent_label = 1; # 1 = beads are labeled, 0 = void space is labeled
    # crop_bool = False
    # channel_num = 1 # 1, 2, 3, 4
    # dx = 0.66  # voxel sizes in the original 3D image (unit: um)
    # dy = 0.66
    # dz = 0.1999952
    # dxyz = 0.8 # voxel size in the resized 3D image (unit: um)

    # img3d = convert_lif(filename, dx, dy, dz, dxyz, channel_num, fluorescent_label, crop_bool)

    # FOR TIF FILES - uncomment below and comment out Lines 130

    filename = r'./input/Rods_3_1_Stack_1A.tif'
    #dx, dy, dz = get_pixel_and_z_dimensions(filename)
    dx = 1.1375; dy = 1.1375; dz = 1
    dxyz = 1.5
    fluorescent_label = 1 # 1 = beads are labeled, 0 = void space is labeled
    crop_bool = 0

    img3d = convert_tif(filename, dx, dy, dz, dxyz, fluorescent_label, crop_bool)
