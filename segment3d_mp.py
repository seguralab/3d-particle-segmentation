...
from utils_mp import find_seeds, refine_seeds, watershed_again, smooth_region, list_sparse # , refine_labels
...
if __name__ == '__main__':
    ...
    # Maximim intensity considered. Intensity higher than "inten_max" will be always 
    # considered foreground, even if its relative intensity is below "-th_relative".
    inten_max = int(th/0.3) # 1000
    # Relative threshold. Voxels below "-th_relative" will be considered background, 
    # even if its absolute intensity is higher than "th".
    th_relative = int(th/3) # 100
    # Example index of frame to plot (used when does_plot == True)
    example_frame = 29
    # Whether further smoothing is applied
    further_smooth = True
    ...
    useMP = True # Whether use multiprocessing to speed up
    ...
    local_maxi = refine_seeds(labels, local_maxi, empty_ext, s2v_max, footprint_direct, footprint_indirect, useMP, p)
    ...
    footprint_direct, footprint_indirect, useMP, p)
    ...
    if further_smooth:
        list_labels_2 = smooth_region(list_labels_2, (Lx,Ly,Lz), footprint_neighbor, useMP, p)
    ...
    (list_bead_voxel_count, list_bead_data) = list_sparse(list_labels_2, (Lx,Ly,Lz), useMP, p)
